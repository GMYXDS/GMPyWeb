...
def headers_to_obj(raw_headers):
    """将原始请求头转为dict字典"""
    return dict([k.strip() for k in i.split(": ", 1)] for i in raw_headers.split('\n'))


def post_data_to_obj(raw_posts):
    """将post数据转为dict字典"""
    data_obj = {}
    for item in raw_posts.split(("&")):
        if item == "":
            continue
        arr = item.split(("="), maxsplit=1)
        # 不处理数字
        # data_obj[arr[0]] = arr[1]
        # 处理数字
        try:
            data_obj[arr[0]] = int(arr[1])
        except Exception:
            data_obj[arr[0]] = arr[1]
    return data_obj
...
if __name__ == '__main__':
    # raw_headers = """"""
    # data = headers_to_obj(raw_headers)
    # print(data)
    # cpprint(data)
    # raw_posts = """"""
    # data = post_data_to_obj(raw_posts)
    # cpprint(data)

    # print(get_user_agent_pc())
    # print(get_user_agent_phone())
    pass